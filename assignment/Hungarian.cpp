...
void
Hungarian::WriteConfig(BipartiteGraph& _bg){

  ofstream config;
  config.open ("assignment_config.txt");
  for(unsigned int i=0; i<_bg.GetNumAgents(); i++){
    for(unsigned int j=0; j<_bg.GetNumTasks(); j++){
      if(_bg.GetMatrix(i,j)->GetMatchedFlag())
      	config<<"  1\t";
...
void
Hungarian::WriteConfig2(const vector<EID>& e){

  ofstream config;
  config.open ("./data/assignment_vector.txt");
...
Hungarian::RetrievePath(void){
  ...
  vector<uint> _starts;
  _starts.push_back(data.size());
  vector<vector<uint> >paths(_starts.size());
  for(uint i=0; i<_starts.size(); i++){
    uint id=_starts[i];
    uint next=data[id];
    paths[i].push_back(next);
    uint cnt = 0;
    while(find(_starts.begin(), _starts.end(), next)==_starts.end()){
      id = next;
      next = data[id];
      paths[i].push_back(next);
      if(++cnt>data.size()){
    ...
    cout<<"Path["<<i<<"] starting from "<<_starts[i]<<":"<<endl;
